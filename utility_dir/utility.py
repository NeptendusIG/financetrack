...
class Settings:
    ...
    def setup_logging(cls, logger_name="main"):
        try:
            import utility_dir.log_config as log  # config: dct et Filtres
...
class GUI:
    ...
    @classmethod
    def set_basic_window(cls, title="Tableau des commandes", level="auto", themename="journal", size="", ignore_activity=False):
        ...
        if not ignore_activity:
            cls.last_active_window = window
            # window.bind("<FocusIn>", lambda event: setattr(cls, "_last_active_window", window))
        return window
