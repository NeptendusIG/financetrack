...
from utility_dir.utility import GUI, Settings, File
from function_dir.csv_managing import import_new_transactions_dt, update_csvfile_with_dataframe
from function_dir.modify_dataframe import separer_types_de_transactions, categoriser_transactions, ajouter_nom_proprietaire
from function_dir.interfaces import get_interval_dates, date_filter, hist_paired_and_stacked, evolution_balance
...
def mise_a_jour_depuis_dossier():
    ...
    new = "FinancesTracker/data/input/CSV_2024-07-09-11.28.csv"
    update_df = import_new_transactions_dt(new, "FinancesTracker/data/output/archives.csv", )
    ...
    update_csvfile_with_dataframe("data/output/archives.csv", update_df, used_headers)
...
def categorisation_des_transactions():
    ...
    df: pd.DataFrame = pd.read_csv("FinancesTracker/data/output/archives.csv", delimiter=';', index_col="num")
    logger.info(f"CategorisingTransactions: LOADED \n\t({df.columns})")
    typing_df = separer_types_de_transactions(df.copy())
    categ_df = categoriser_transactions(typing_df, dict_path="FinancesTracker/names.json")
    owner_df = ajouter_nom_proprietaire(categ_df,  dict_path="FinancesTracker/names.json")
    logger.info(f"CategorisingTransactions: DONE \n\t({owner_df.columns})")
    owner_df.to_csv("FinancesTracker/data/output/completed.csv", sep=';')
...
def montrer_graph_resultats():
    ...
    # Données
    df = pd.read_csv("FinancesTracker/data/output/completed.csv", delimiter=';')
...
if __name__ == '__main__':
    # - Variables -
    # - Environment -
    File.create_file("FinancesTracker/data/output/archives.csv", can_make_dirs=False, default_content=";".join(used_headers))
    # - Programme -
